...
router.get('/', function (req, res) {
  ...
  pool.connect()
    .then(function (client) {
      client.query('SELECT heroes.*, super_powers.name, super_powers.description ' +
      'FROM heroes JOIN super_powers ON heroes.power_id = super_powers.id ' +
      'WHERE heroes.id=$1', [heroId])
